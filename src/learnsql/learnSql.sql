...
SELECT
  ...
  E1.LAST_NAME,
  E2.LAST_NAME
FROM employee_tbl E1, employee_tbl E2
...
WHERE CUST_ID IN (
  ...
  WHERE
    O.PROD_ID = P.PROD_ID AND O.QTY * P.COST < (
      SELECT SUM(COST)
      FROM products_tbl
    )
);
...
SELECT
  C.CUST_ID,
  C.CUST_NAME,
  (O.QTY * P.COST) CUST_COST
FROM
  customer_tbl C INNER JOIN orders_tbl O ON C.CUST_ID = O.CUST_ID
...
INSERT INTO orders_tbl VALUES ('AM8798', '090', '1234', 9, '2013-05-06');
INSERT INTO orders_tbl VALUES ('AM8776', '287', '11235', 2, '2012-05-06');

SELECT SUM(O.QTY)
FROM orders_tbl O, customer_tbl C
WHERE O.CUST_ID = C.CUST_ID
GROUP BY O.CUST_ID;
...
SET C.CUST_NAME = 'DAVIDS MARKET'
WHERE C.CUST_ID = (
  SELECT O.CUST_ID
  FROM orders_tbl O
  WHERE O.ORD_NUM = '23E934'
);
...
INSERT INTO products_tbl VALUES ('907', 'TRUFFLES CANDY', 8.8);

SELECT
  O.QTY,
  P.COST,
  (O.QTY * P.COST) SUM_PRICES
FROM orders_tbl O, products_tbl P
WHERE O.PROD_ID = P.PROD_ID;

SELECT SUM(O.QTY * P.COST) / COUNT(DISTINCT CUST_ID) AVG_COST
FROM orders_tbl O, products_tbl P
WHERE O.PROD_ID = P.PROD_ID;
...
SELECT
  C.CUST_ID,
  C.CUST_NAME,
  SUM(O.QTY * P.COST) CUST_COST
FROM customer_tbl C, orders_tbl O, products_tbl P
WHERE C.CUST_ID = O.CUST_ID AND O.PROD_ID = P.PROD_ID
GROUP BY C.CUST_ID;
...
WHERE C.CUST_ID = O.CUST_ID AND O.PROD_ID = P.PROD_ID
GROUP BY C.CUST_ID
HAVING CUST_COST > (SELECT SUM(O.QTY * P.COST) / COUNT(DISTINCT CUST_ID) AVG_COST
                    FROM orders_tbl O, products_tbl P
                    WHERE O.PROD_ID = P.PROD_ID);
...
# CHAPTER 15
UPDATE employee_pay_tbl SET PAY_RATE = '18.16' WHERE EMP_ID LIKE '4436%';
...
SELECT 'CUSTOMERS' TYPE, COUNT(*) FROM customer_tbl
UNION
...
SELECT C.CUST_ID, C.CUST_NAME FROM customer_tbl C
WHERE C.CUST_ID NOT IN (SELECT O.CUST_ID FROM orders_tbl O);

SELECT C.CUST_ID, C.CUST_NAME FROM customer_tbl C
WHERE C.CUST_ID IN (SELECT O.CUST_ID FROM orders_tbl O);
